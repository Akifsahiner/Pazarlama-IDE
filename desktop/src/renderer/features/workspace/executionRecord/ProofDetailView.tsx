...
export function ProofDetailView() {
    ...
    return (
      <div className="flex h-full min-h-[160px] items-center justify-center p-6 text-center">
        <p className="max-w-sm text-body-sm text-text-3">
          Kanıt henüz girilmedi. User task veya operator proof tamamlandığında burada görünür.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 overflow-y-auto p-4">
      <div>
        <div className="text-mini font-semibold uppercase tracking-wide text-text-3">Görev</div>
      ...
      {proof.note && (
        <div className="rounded-[var(--radius-md)] border border-accent/20 bg-accent-soft/10 p-3">
          <div className="text-mini font-semibold text-text-3">Not</div>
      ...
      {proof.completed_at && (
        <p className="text-micro text-text-3">
          Tamamlandı · {new Date(proof.completed_at).toLocaleString()}
        </p>
      )}
    </div>
  );
