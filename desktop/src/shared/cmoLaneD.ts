  ...
  const tasks: CmoOpsTask[] = [
    {
      ...
      day_slot: "now",
      unlocked_at: now,
      deliverable: `Activation event "${input.activation.activation_event_label}" wired in repo or manual baseline doc`,
      execution_mode: "repo_edit",
      estimated_effort_minutes: 90,
      if_failed: "If event cannot fire → log manual baseline and open PRODUCT REQUEST",
      day_offset: 0,
      inputs: [{ label: "Activation event", value: input.activation.activation_event_label }],
      required_proof: ["browser_evidence"],
      depends_on: [],
      when: { day_offset: 0, slot: "now", due_at: now },
    },
    {
      ...
      status: "pending",
      day_slot: "up_next",
      deliverable: "Onboarding path diff with activation tracking in preview",
      execution_mode: "repo_edit",
      estimated_effort_minutes: 120,
      if_failed: "If scope too large → ship smallest path that reaches first value",
      day_offset: 2,
      inputs: [{ label: "TTFV target", ref: "activation.ttfv_minutes" }],
      required_proof: ["browser_evidence"],
      depends_on: [`${input.thesis.id}.product.w${weekIndex}.0`],
      when: { day_offset: 2, slot: "up_next" },
    },
    {
      ...
      status: "pending",
      day_slot: "later",
      deliverable: "Issue/PR URL + activation metric snapshot",
      execution_mode: "product_request",
      estimated_effort_minutes: 40,
      if_failed: "If activation flat → escalate to core workflow fix before resuming marketing",
      day_offset: 4,
      inputs: [{ label: "Product request export", ref: "lane_d.export" }],
      required_proof: ["live_url", "kpi"],
      metric: {
        id: "activation_rate",
        name: "Activation rate",
        target: input.activation.activation_rate_target_pct,
        unit: "%",
        measurable: true,
      },
      depends_on: [`${input.thesis.id}.product.w${weekIndex}.1`],
      when: { day_offset: 4, slot: "later" },
    },
  ];
