...
export function buildDelegateSowMarkdown(block: DelegateHireBlock, brief: CmoDelegateBrief): string {
  const budgetLine = block.cost_estimate_usd
    ? `**Trial budget cap:** $${block.cost_estimate_usd} USD (from budget plane)`
    : "";
  return [
    `# Statement of Work — ${block.job_title}`,
    ``,
    `## Scope`,
    brief.what,
    ``,
    ...
    ``,
    `## Trial KPIs`,
    ...block.trial_kpis.map((k) => `- ${k.label}: ${k.target} ${k.unit}`),
    ``,
    `## Compensation & hours`,
    block.compensation_frame,
    block.hours_per_week ? `~${block.hours_per_week}h/week during trial` : "",
    budgetLine,
    ``,
    `## Milestones`,
    `- Day 1: Handoff + rubric schedule confirmed`,
    `- Daily: Rubric check-in with proof URL or note`,
    `- End of trial: KPI rollup + promote/release verdict`,
    ``,
    `## IP & confidentiality`,
    `Work product belongs to the hiring founder. Contractor may not reuse deliverables for other clients without written consent.`,
    ``,
    `## Termination`,
    `Either party may end the trial with 24h notice. Partial rubric days are paid pro-rata per agreed frame.`,
    ``,
    `---`,
    `Generated by Marketing IDE · delegate SOW`,
  ]
    .filter(Boolean)
    .join("\n");
}

const EXTENSION_DAY_CHECKLIST: RubricCheckItem[] = [
  { id: "deliverable", label: "Deliverable submitted with proof URL or note", required: true },
  { id: "sync", label: "Sync note with founder (8+ chars)", required: true },
];

export function extendDelegateTrial(
  ...
  if (!brief) return workspace;

  const existing = workspace.daily_rubrics.filter((r) => r.brief_id === briefId);
  const maxDay = existing.reduce((m, r) => Math.max(m, r.day_index), 0);
  const newRubrics: DelegateDailyRubric[] = [];

  for (let i = 1; i <= extraDays; i++) {
    const dayIndex = maxDay + i;
    newRubrics.push({
      id: `${briefId}.rubric.ext${dayIndex}`,
      brief_id: briefId,
      day_index: dayIndex,
      title: `Extension day ${i} — tighter checklist`,
      checklist: EXTENSION_DAY_CHECKLIST.map((c) => ({ ...c })),
      status: "pending",
    });
  }

  const done = existing.filter((r) => r.status === "done").length;
  const total = existing.length + extraDays;

  void thesis;
  return {
    ...workspace,
    daily_rubrics: [...workspace.daily_rubrics, ...newRubrics],
    verdict: {
      kind: "on_track",
      brief_id: briefId,
      headline: `Trial extended +${extraDays} days — tighter checklist`,
      rationale: [
        `Added ${extraDays} rubric days after partial delivery.`,
        "Continue daily check-ins until promote threshold met.",
      ],
      evidence: [`${done}/${total} rubric days complete`],
...
function rubricTemplateForKind(kind: HireBriefKind, briefId: string, weekIndex: number): Omit<DelegateDailyRubric, "status">[] {
    ...
    return [
      {
        ...
        checklist: [
          { id: "outline", label: "Outline doc shared with founder", required: true },
          { id: "angle", label: "Hook angle confirmed", required: true },
        ],
      },
      {
        id: `${briefId}.rubric.d2`,
        ...
        checklist: [
          { id: "draft", label: "Full draft in doc (URL)", required: true },
          { id: "cta", label: "CTA matches Lane B copy", required: true },
        ],
      },
      {
        id: `${briefId}.rubric.d3`,
        ...
        title: "Publish-ready",
        checklist: [
          { id: "ready", label: "Final copy approved for publish", required: true },
          { id: "assets", label: "Images/links attached", required: true },
        ],
      },
      {
        id: `${briefId}.rubric.d4`,
        brief_id: briefId,
        day_index: 4,
        title: "QA + handback",
        checklist: [
          { id: "published", label: "Published URL or scheduled proof", required: true },
          { id: "metrics", label: "Initial metrics note (8+ chars)", required: true },
        ],
      },
    ];
  }
  ...
  if (kind === "agency_wave") {
    return Array.from({ length: 5 }, (_, i) => ({
      ...
      brief_id: briefId,
      day_index: i + 1,
      title: i < 4 ? `Day ${i + 1} — 4 DMs logged` : "Response summary handback",
      checklist:
        i < 4
          ? [
              { id: "dms", label: "4 personalized DMs logged", required: true },
              { id: "utm", label: "UTM links used where provided", required: true },
            ]
          : [
              { id: "summary", label: "Response summary with counts", required: true },
              { id: "handback", label: "Creator response notes per target", required: true },
            ],
    }));
  }
