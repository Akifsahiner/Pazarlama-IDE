...
describe("executionRecord", () => {
  it("buildBottleneckSentence formats Turkish one-liner", () => {
    const s = buildBottleneckSentence({
      bottleneck: "Dağıtım hacmi darboğaz",
      nextMove: "Landing + founder outreach",
    });
    assert.match(s, /Darboğaz Dağıtım hacmi darboğaz → sıradaki hareket Landing \+ founder outreach/);
  });
  ...
  it("buildActiveExecutionRecord composes goal, experiment, lifecycle", () => {
    ...
    assert.match(record.bottleneckSentence, /Darboğaz/);
