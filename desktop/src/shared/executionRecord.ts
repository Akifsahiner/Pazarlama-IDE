...
export function buildBottleneckSentence(input: {
  ...
  if (!bottleneck && !nextMove) return "Darboğaz belirleniyor → sıradaki hareket planlanıyor";
  if (!nextMove) return `Darboğaz ${bottleneck} → sıradaki hareket belirleniyor`;
  if (!bottleneck) return `Darboğaz belirleniyor → sıradaki hareket ${nextMove}`;
  return `Darboğaz ${bottleneck} → sıradaki hareket ${nextMove}`;
}
...
function resolveGoal(input: BuildActiveExecutionRecordInput): string {
  ...
  return "Büyüme hedefi tanımlanmadı";
}
...
function resolveExperimentFromInput(
  ...
  return "Week 1 deneyi başlatılacak";
}
...
function formatDoneFromProof(proof: CmoOpsProof, task: CmoOpsTask): ExecutionDoneItem[] {
  ...
  if (proof.urls?.length) {
    for (const [i, url] of proof.urls.entries()) {
      items.push({ id: `${task.id}-url-${i}`, label: "Kanıt URL", detail: url });
    }
  }
